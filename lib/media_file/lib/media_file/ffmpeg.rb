...
module Danbooru
  class FFmpeg
    ...
    def smart_video_preview!
      ...
      Danbooru::FFmpeg.shell!("ffmpeg -i #{file.path.shellescape} -vf thumbnail=300 -frames:v 1 -y #{vp.path.shellescape}")
    ...
    memoize def metadata
      output = Danbooru::FFmpeg.shell!("ffprobe -v quiet -print_format json -show_format -show_streams -show_packets -show_frames #{file.path.shellescape}")
      JSON.parse(output) || {}
    rescue Error => e
    ...
    memoize def container_duration
      metadata.dig(:format, :duration).presence&.to_f
    end
    ...
    memoize def playback_info
      ...
      output, progress = Danbooru::Tempfile.create do |tempfile|
        output = Danbooru::FFmpeg.shell!("ffmpeg -hide_banner -i #{file.path.shellescape} -progress #{tempfile.path} -af silencedetect=noise=0.0001:duration=0.25s,ebur128=metadata=1:dualmono=true:peak=true -f null /dev/null")
